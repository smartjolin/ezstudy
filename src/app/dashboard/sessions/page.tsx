    ...
    <div>
      ...
      <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() => {
              // Generate iCal file for all sessions
              const events = sessions
                ...
                .map((s) => {
                  ...
                  return [
                    ...
                    `DTSTART:${fmt(start)}`,
                    `DTEND:${fmt(end)}`,
                    `SUMMARY:${s.title || "Tutoring Session"}`,
                    `DESCRIPTION:Student: ${s.student.name || s.student.email}`,
                    `UID:${s.id}@ezstudy`,
                    "END:VEVENT",
                  ].join("\r\n");
                });
              ...
              a.href = url;
              a.download = "ezstudy-sessions.ics";
              a.click();
...
function SessionCard({
  ...
  const [recapLanguage, setRecapLanguage] = useState<string>("en");
  ...
  const generateRecap = async () => {
    ...
    try {
      const res = await fetch(`/api/sessions/${session.id}/recap`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ notes, language: recapLanguage }),
      });
      if (res.ok) {
        const data = await res.json();
            ...
            <div className="mt-2 flex items-center gap-2">
              ...
              {session.status === "COMPLETED" && (
                <>
                  <select
                    value={recapLanguage}
                    onChange={(e) => setRecapLanguage(e.target.value)}
                    className="rounded-lg border border-primary-light/20 bg-white px-2 py-1.5 text-xs text-text-secondary outline-none"
                  >
            ...
            <div className="rounded-xl border border-accent-light/30 bg-gradient-to-br from-primary/5 to-accent/5 p-5">
              <div className="mb-3 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => {
                      navigator.clipboard.writeText(session.recap || "");
                      setRecapCopied(true);
                      setTimeout(() => setRecapCopied(false), 2000);
                    }}
                    className="flex items-center gap-1 rounded-lg border border-primary-light/20 bg-white px-2.5 py-1.5 text-xs font-medium text-text-secondary transition-all hover:text-primary-dark"
                  >
                    {recapCopied ? (
                  ...
                  <a
                    href={`mailto:?subject=${encodeURIComponent(
                      `Session Recap: ${session.title || "Tutoring Session"} — ${format(scheduledDate, "MMM d, yyyy")}`
                    )}&body=${encodeURIComponent(
                      `Hi,\n\nHere is the recap from ${
                        session.student.name || "your child"
                      }'s tutoring session on ${format(scheduledDate, "MMMM d, yyyy")}:\n\n${session.recap}\n\nBest regards`
                    )}`}
                    className="flex items-center gap-1 rounded-lg border border-primary-light/20 bg-white px-2.5 py-1.5 text-xs font-medium text-text-secondary transition-all hover:text-primary-dark"
                  >
